...
export default async function UsersPage() {
  ...
  const UserCard = ({ user, isActive }: { user: any, isActive: boolean }) => {
    ...
    // ИСПРАВЛЕНИЕ: Создаем очищенный объект без BigInt (telegramId), 
    // чтобы Next.js не падал с 500-й ошибкой при передаче в Client Component
    ...
    return (
      <div className={`border border-gray-200 bg-white p-5 rounded-lg shadow-sm flex flex-col justify-between ${!isActive ? 'opacity-60 grayscale' : ''}`}>
        ...
        <div className="flex gap-4 items-center mt-2 border-t pt-2 flex-wrap">
          ...
          {/* ИСПРАВЛЕНИЕ: Передаем безопасный объект */}
