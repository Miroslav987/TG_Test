...
import { createProject, addUserToProject, createTask, toggleProjectStatus } from "../../actions/projects";
import DeleteProjectButton from "@/components/DeleteProjectButton";
import ProjectUserBadge from "../../components/ProjectUserBadge"; // <-- ДОБАВЛЕНО
import ProjectTaskItem from "../../components/ProjectTaskItem";   // <-- ДОБАВЛЕНО
...
export default async function ProjectsPage() {
  ...
  const ProjectCard = ({ project }: { project: any }) => {
    const availableUsers = allUsers.filter(u => !project.users.some((pu: any) => pu.id === u.id));
    const canDelete = project._count.tasks === 0 && project._count.reports === 0;

    return (
      <div className={`bg-white border p-6 rounded-lg shadow-sm ${!project.isActive ? 'opacity-70 bg-gray-50' : ''}`}>
        <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4 border-b pb-4">
          <h2 className="text-xl font-bold">{project.name} {!project.isActive && "(Архив)"}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {/* ЗАМЕНЕНО НА КЛИЕНТСКИЙ КОМПОНЕНТ */}
              {project.users.map((u: any) => (
                <ProjectUserBadge key={u.id} user={u} projectId={project.id} />
              ))}
          ...
          <div>
            ...
            <div className="mb-4">
              ...
              {project.tasks.map((task: any) => (
                <ProjectTaskItem key={task.id} task={task} projectUsers={project.users} />
              ))}
              ...
              <form action={createTask} className="bg-gray-50 p-3 border rounded text-sm space-y-2 mt-4">
                ...
                <div className="flex gap-2">
                  <select name="assigneeId" className="p-1.5 border rounded flex-1">
                    ...
                    {project.users.map((u: any) => <option key={u.id} value={u.id}>{u.name}</option>)}
                  </select>
